...
class MainController {
  ...
  static liveVideo(
    ...
    const streamVideo = (streamId) => {
      ...
      MainController.streams.set(streamId, {
  ...
  static async stopLiveVideoToken(streamId) {
    try {
      ...
      if (MainController.streams.has(streamId)) {
        ...
        return res.json({
          status: "success",
          message: `Live stream ${streamId} stopped`,
        });
      } else {
        return res
          .status(400)
          .json({ error: `No active live stream found with ID ${streamId}` });
      }
    } catch (error) {
      console.error("Error ending live video:", error.message);
      return res
        .status(500)
        .json({ error: "Failed to end live video", details: error.message });
    }
  }
      ...
      try {
        ...
        const streamVideo = () => {
          const ffmpegCommand = ffmpeg(videoPath)
            ...
            .on("start", (commandLine) => {
              console.log(
                `Stream ${streamId} started with command:`,
                commandLine
              );
            })
            .on("error", (err) => {
